...
import { Fonts, FontSizes, Spacing, Radius, createShadows, type ColorPalette } from '../../../../src/constants/theme';
...
function createStyles(theme: ColorPalette) {
  ...
  return StyleSheet.create({
    ...
    actionBtnDelete: { backgroundColor: 'rgba(255,100,100,0.25)' },
    actionBtnText: {
    ...
    heroRight: { flex: 1, justifyContent: 'flex-end', paddingBottom: 20 },
    avatarRing: {
      borderRadius: Radius.full,
      padding: 4,
    ...
    heroBadgesRow: { flexDirection: 'row', justifyContent: 'center', gap: Spacing.md },
    balanceBadge: {
      flexDirection: 'row',
      alignItems: 'center',
      gap: Spacing.xs,
...
export default function ReaderDashboardScreen() {
  ...
  const id = selectedReader?.id ?? null;
  const { deleteReader } = useReaders();
  const showDialog = useDialogStore((s) => s.show);
  const { books, isLoading, refresh, fetchNextPage, hasNextPage, isFetchingNextPage } = useBooks(id);
  const { readingBooks, refresh: refreshReading } = useReadingBooks(id);
  const { streak } = useStreak(id);
  const { earnedBadges, refresh: refreshBadges } = useBadges(id);
  const theme = useTheme();
  const styles = useMemo(() => createStyles(theme), [theme]);
  const { gesture: swipeGesture } = useTabSwipe('reader');
