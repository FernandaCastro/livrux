const pt = {
  common: {
    appName: 'Livrux',
    save: 'Salvar',
    cancel: 'Cancelar',
    delete: 'Excluir',
    edit: 'Editar',
    confirm: 'Confirmar',
    loading: 'Carregando...',
    error: 'Ocorreu um erro',
    retry: 'Tentar novamente',
    back: 'Voltar',
    next: 'Próximo',
    done: 'Concluído',
    or: 'ou',
    ok: 'OK',
    errorBoundaryTitle: 'Algo deu errado',
    errorBoundarySubtitle: 'O app encontrou um erro inesperado. Tente novamente.',
  },
  auth: {
    signIn: 'Entrar',
    signUp: 'Cadastrar',
    signOut: 'Sair',
    email: 'E-mail',
    password: 'Senha',
    confirmPassword: 'Confirmar senha',
    forgotPassword: 'Esqueceu a senha?',
    resetPassword: 'Redefinir senha',
    noAccount: 'Não tem uma conta?',
    hasAccount: 'Já tem uma conta?',
    signInWithGoogle: 'Continuar com o Google',
    signInWithApple: 'Continuar com a Apple',
    name: 'Seu nome',
    emailPlaceholder: 'voce@exemplo.com',
    passwordPlaceholder: 'Pelo menos 8 caracteres',
    sendCode: 'Enviar código',
    codeSent: 'Um código de 6 dígitos foi enviado para seu e-mail. Insira-o abaixo junto com sua nova senha.',
    verificationCode: 'Código de verificação',
    verificationCodePlaceholder: '123456',
    newPassword: 'Nova senha',
    savePassword: 'Salvar senha',
    passwordChanged: 'Senha alterada',
    passwordChangedBody: 'Sua senha foi alterada com sucesso.',
    resetSuccess: 'Senha redefinida com sucesso. Entre com sua nova senha.',
    resendCode: 'Reenviar código',
    resendCodeIn: 'Reenviar código ({{seconds}}s)',
    confirmationEmailTitle: 'Verifique sua caixa de entrada',
    confirmationEmailBody: 'Um e-mail de confirmação foi enviado. Verifique seu e-mail antes de entrar.',
    resendConfirmationEmail: 'Reenviar e-mail',
    resendConfirmationEmailIn: 'Reenviar e-mail ({{seconds}}s)',
    resendConfirmationEmailSent: 'E-mail enviado!',
    termsAgree: 'Li e aceito os',
    termsLink: 'Termos de Uso',
    termsAnd: 'e a',
    privacyLink: 'Política de Privacidade',
    errors: {
      invalidEmail: 'Insira um endereço de e-mail válido.',
      weakPassword: 'A senha deve ter pelo menos 8 caracteres.',
      passwordTooShort: 'A senha deve ter pelo menos 8 caracteres.',
      passwordMismatch: 'As senhas não coincidem.',
      invalidCredentials: 'E-mail ou senha incorretos.',
      emailInUse: 'Este e-mail já está cadastrado.',
      generic: 'Algo deu errado. Por favor, tente novamente.',
      invalidCode: 'Código inválido ou expirado.',
      termsRequired: 'Você deve aceitar os Termos de Uso e a Política de Privacidade.',
    },
  },
  home: {
    title: 'Leitores',
    addReader: 'Adicionar Leitor',
    emptyTitle: 'Nenhum leitor ainda',
    emptySubtitle: 'Adicione um leitor e comece a registrar livros!',
    balance: '{{count}} Livrux',
  },
  reader: {
    newReader: 'Novo Leitor',
    editReader: 'Editar Leitor',
    readerName: 'Nome do Leitor',
    readerNamePlaceholder: 'ex: João',
    refreshAvatar: 'Novo avatar',
    deleteReader: 'Excluir Leitor',
    deleteConfirm: 'Excluir {{name}}? Todos os livros e Livrux serão perdidos.',
    balance: 'Saldo',
    books: 'Livros lidos',
    readingNow: 'Lendo agora',
    noBooks: 'Nenhum livro ainda!',
    noBooksHint: 'Registre o primeiro livro lido e ganhe moedas! 🪙',
    logFirstBook: 'Registre o primeiro livro!',
  },
  book: {
    logBook: 'Registrar livro',
    editBook: 'Editar livro',
    title: 'Título',
    titlePlaceholder: 'ex: Harry Potter',
    author: 'Autor',
    authorPlaceholder: 'ex: J.K. Rowling',
    totalPages: 'Total de páginas',
    totalPagesPlaceholder: 'ex: 320',
    dateCompleted: 'Data de conclusão',
    coverPhoto: 'Foto da capa',
    addCover: 'Adicionar capa',
    changeCover: 'Alterar capa',
    willEarn: 'Vai ganhar',
    livruxCoins: '{{count}} Livrux',
    deleteBook: 'Excluir livro',
    deleteConfirm: 'Excluir "{{title}}"? Os Livrux ganhos e sessões de leitura serão removidos.',
    badgesRevoked: 'Você perdeu conquistas!',
    badgesRevokedSubtitle: 'Ao excluir esse livro, você não se qualifica mais para:',
    badgesRevokedEncouragement: 'Continue lendo e você vai reconquistá-las! 💪',
    badgesRevokedBody: 'Você perdeu as conquistas: {{names}}.',
    searchPlaceholder: 'Buscar por título ou autor…',
    fillManually: 'preencher manualmente',
    loadMore: 'Carregar mais resultados',
    duplicateTitle: 'Livro já registrado',
    duplicateMessage: 'Este livro já foi registrado para este leitor. Deseja registrar novamente como uma releitura?',
    duplicateConfirm: 'Sim, registrar',
    pagesCount: '{{count}} páginas',
    scanHint: 'Aponte para o código de barras do livro',
    foreignLanguage: 'Livro em língua estrangeira',
    ratingLabel: 'O que achou?',
    ratingDisliked: 'Não gostei',
    ratingLiked: 'Gostei',
    ratingLoved: 'Amei',
    reviewLabel: 'Opinião',
    reviewPlaceholder: 'Escreva algo sobre o livro…',
    statusCompleted: 'Já terminei',
    statusReading: 'Estou lendo',
    dateStart: 'Data de início',
    logReading: 'Começar a ler',
    completeBook: 'Concluir livro',
    completedOn: 'Concluído em',
    startedOn: 'Iniciado em',
    readingDays: '{{days}} dias lendo',
    errors: {
      titleRequired: 'O título é obrigatório.',
      pagesRequired: 'O total de páginas é obrigatório.',
      pagesInvalid: 'As páginas devem ser um número positivo.',
      dateRequired: 'A data é obrigatória.',
      dateInvalid: 'Informe uma data válida (DD/MM/AAAA).',
      searchUnavailable: 'Busca indisponível no momento. Tente novamente mais tarde ou preencha manualmente.',
      searchFailed: 'Não foi possível acessar a busca de livros. Verifique sua conexão e tente novamente.',
    },
  },
  rewards: {
    title: 'Carteira',
    totalBalance: 'Saldo total',
    history: 'Histórico',
    emptyHistory: 'Nenhuma transação ainda.',
    earned: '+{{amount}} Livrux',
    spent: '-{{amount}} Livrux',
    spendButton: 'Gastar Livrux',
    reasonBookCompleted: 'Livro lido',
    reasonBookDeleted: 'Livro excluído',
    reasonBookUpdated: 'Livro atualizado',
    reasonManualSpend: 'Gasto',
  },
  spend: {
    title: 'Gastar Livrux',
    amount: 'Quanto gastou?',
    amountPlaceholder: 'ex: 10',
    description: 'Onde gastou?',
    descriptionPlaceholder: 'ex: Sorvete na padaria',
    submit: 'Registrar gasto',
    availableBalance: 'Saldo disponível',
    errors: {
      amountRequired: 'Informe o valor gasto.',
      amountInvalid: 'O valor deve ser maior que zero.',
      amountExceedsBalance: 'Saldo insuficiente.',
      descriptionRequired: 'Descreva onde foi gasto.',
    },
  },
  parental: {
    title: 'Controle Parental',
    parentalPinSection: 'PIN Parental',
    setParentalPin: 'Definir PIN parental',
    changeParentalPin: 'Alterar PIN parental',
    removeParentalPin: 'Remover PIN parental',
    unlockDuration: 'Tempo de desbloqueio',
    readerPinsSection: 'PINs dos leitores',
    noReaders: 'Nenhum leitor cadastrado.',
    setPinBtn: 'Definir PIN',
    changePinBtn: 'Alterar',
    confirmCurrentToRemove: 'Digite o PIN atual para removê-lo',
    confirmCurrentToChange: 'Digite o PIN atual',
    enterNewPin: 'Digite o novo PIN',
    confirmNewPin: 'Digite novamente para confirmar',
    removeReaderPinTitle: 'Remover PIN',
    removeReaderPinConfirm: 'Remover o PIN de {{name}}?',
    confirmReaderPin: 'Digite o PIN atual de {{name}}',
  },
  settings: {
    title: 'Configurações',
    parentalControls: 'Família',
    parentalControlsTitle: 'Controle parental',
    formula: 'Fórmula de recompensa',
    formulaTitle: 'Fórmula Livrux',
    baseReward: 'Recompensa base (por livro)',
    perPageRate: 'Moedas por página',
    preview: 'Um livro de {{pages}} páginas rende {{coins}} Livrux',
    account: 'Conta',
    displayName: 'Nome de exibição',
    displayNamePlaceholder: 'ex: Maria',
    displayNameRequired: 'O nome é obrigatório.',
    displayNameTooLong: 'O nome deve ter no máximo 50 caracteres.',
    displayNameSaved: 'Nome atualizado',
    displayNameSavedBody: 'Seu nome de exibição foi atualizado com sucesso.',
    changePassword: 'Alterar senha',
    language: 'Idioma',
    deleteAccount: 'Excluir conta',
    deleteAccountConfirm: 'Isso excluirá permanentemente sua conta, todos os leitores e dados. Essa ação não pode ser desfeita.',
    deleteAccountFinal: 'Confirmar exclusão',
    deleteAccountFinalBody: 'Tem certeza absoluta? Todos os livros, Livrux e histórico serão perdidos para sempre.',
    signOut: 'Sair',
    legal: 'Legal',
    manual: 'Manual do Usuário',
    privacyPolicy: 'Política de Privacidade',
    termsOfUse: 'Termos de Uso',
    bonusRules: 'Regras de bônus',
    minPagesBonus: 'Bônus por páginas mínimas',
    pageThreshold: 'Mínimo de páginas',
    foreignLanguageBonus: 'Bônus por língua estrangeira',
    bonusAmount: 'Bônus (Livrux)',
  },
  streak: {
    title: 'Sequência',
    current: '🔥 {{count}} dia',
    current_plural: '🔥 {{count}} dias',
    best: 'Recorde: {{count}} dias',
    logSession: '📖 Li hoje',
    alreadyLoggedToday: '✓ Leitura de hoje registrada!',
    pagesReadLabel: 'Em qual página você parou?',
    pagesReadPlaceholder: 'ex: 150',
    lastPageHint: 'Última página registrada: {{page}}',
    tapToUpdate: 'toque para atualizar',
    logSessionConfirm: 'Registrar',
    errorInvalidPage: 'Digite um número de página válido.',
    errorPageNotAdvanced: 'A página deve ser maior que a última registrada ({{page}}).',
    errorPageTooHigh: 'Para concluir o livro use o botão "Concluir". Máx: {{total}} páginas.',
  },
  badges: {
    title: 'Conquistas',
    earned: 'Conquistadas',
    pending: 'Para conquistar',
    bonusEarned: '+{{amount}} Livrux',
    bonusXp: '+{{amount}} XP',
    unlocked: 'Você ganhou uma conquista!',
    empty: 'As conquistas aparecem aqui depois de registrar seus primeiros livros!',
    friendEmpty: 'Este leitor ainda não desbloqueou nenhuma conquista.',
    first_book: { name: 'Primeiro Livro', description: 'Registrou o primeiro livro' },
    bookworm_5: { name: 'Amigo dos Livros', description: 'Leu 5 livros' },
    bookworm_25: { name: 'Bibliófilo', description: 'Leu 25 livros' },
    centurion: { name: 'Centurião', description: 'Leu 100 livros' },
    page_hunter_500: { name: 'Caçador de Páginas', description: '500 páginas lidas' },
    page_hunter_5000: { name: 'Devorador de Páginas', description: '5.000 páginas lidas' },
    polyglot: { name: 'Poliglota', description: 'Leu 3 livros em língua estrangeira' },
    streak_7: { name: 'Semana de Fogo', description: '7 dias seguidos lendo' },
    streak_30: { name: 'Mês Incansável', description: '30 dias seguidos lendo' },
    book_club: { name: 'Clube do Livro', description: 'Leu o mesmo livro que um amigo' },
  },
  ranking: {
    title: 'Ranking',
    you: 'eu',
    yourPosition: '{{position}}º lugar',
    noFriends: 'Sem rivais por enquanto! 🤔',
    noFriendsHint: 'Adicione amigos e descubra quem lê mais na turma! 🏆',
  },
  languages: {
    en: 'Inglês',
    de: 'Alemão',
    pt: 'Português',
  },
  guardians: {
    menuLabel: 'Co-responsáveis',
    title: 'Co-responsáveis',
    subtitle: 'Qualquer responsável adicionado pode ver e gerenciar o app da mesma forma que você.',
    members: 'Membros da família',
    pendingInvitations: 'Convites pendentes',
    pendingStatus: 'Convite enviado · aguardando',
    unnamed: 'Responsável',
    thisGuardian: 'este responsável',
    inviteBtn: 'Convidar responsável',
    haveInvite: 'Tenho um convite',
    inviteTitle: 'Convidar responsável',
    inviteSubtitle: 'Um link de convite será enviado para o e-mail informado.',
    sendInvite: 'Enviar convite',
    inviteSentTitle: 'Convite enviado!',
    inviteSentBody: 'O convite foi enviado para {{email}}.',
    invalidEmail: 'Informe um e-mail válido.',
    acceptTitle: 'Aceitar convite',
    acceptSubtitle: 'Insira o código que você recebeu no e-mail de convite.',
    tokenPlaceholder: 'Código do convite',
    tokenRequired: 'Insira o código do convite.',
    acceptBtn: 'Entrar na família',
    acceptedTitle: 'Bem-vindo(a)!',
    acceptedBody: 'Você agora faz parte desta família no Livrux.',
    removeTitle: 'Remover responsável',
    removeConfirm: 'Remover {{name}} da família?',
    leaveTitle: 'Sair da família',
    leaveConfirm: 'Você deixará de ter acesso aos leitores e livros desta família.',
    leaveBtn: 'Sair da família',
    cancelInviteTitle: 'Cancelar convite',
    cancelInviteConfirm: 'Cancelar o convite enviado para {{email}}?',
  },
  friends: {
    title: 'Amigos',
    myCode: 'Meu código',
    shareCodeHint: 'Compartilhe com seus amigos para se conectar',
    codeCopied: '✓ Copiado!',
    myFriends: 'Meus amigos',
    pendingRequests: 'Pedidos de amizade',
    noFriends: 'Nenhum amigo ainda',
    noFriendsHint: 'Compartilhe seu código e adicione amigos para ver os livros deles!',
    booksRead: 'livros lidos',
    addFriend: 'Adicionar amigo',
    enterCodeLabel: 'Digite o código do amigo',
    codePlaceholder: 'EX: ABC123',
    search: 'Buscar',
    sendRequest: 'Pedir amizade',
    requestSentSuccess: 'Pedido enviado! Aguarde a confirmação.',
    codeNotFound: 'Código não encontrado. Verifique e tente novamente.',
    tooManySearches: 'Muitas buscas em pouco tempo. Tente novamente mais tarde.',
    cannotAddSelf: 'Você não pode adicionar a si mesmo.',
    requestAlreadySent: 'Pedido já enviado para este leitor.',
    unfriend: 'Desfazer amizade',
    unfriendTitle: 'Desfazer amizade',
    unfriendConfirm: 'Desfazer amizade com {{name}}?',
    parentApprovalNeeded: 'O pai precisa estar desbloqueado para aceitar pedidos.',
    newRequest: 'Novo pedido!',
    sentRequests: 'Aguardando resposta',
    awaitingResponse: 'Aguardando resposta',
    noBooksSub: 'ainda não leu nenhum livro. Em breve vai ter novidades! 🌟',
    friendsAutonomy: 'Autonomia para amigos',
    friendsAutonomyHint: 'Permite que o leitor aceite pedidos de amizade sozinho',
  },
};

export default pt;
